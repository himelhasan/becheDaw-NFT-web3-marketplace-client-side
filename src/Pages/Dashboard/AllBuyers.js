...
const AllBuyers = () => {
  const { data: users = [], refetch } = useQuery({
    queryKey: ["users"],
    queryFn: async () => {
      const res = await fetch("https://bechedaw-server.vercel.app/all-buyers");
  ...
  const deleteUser = (id) => {
    fetch(`https://bechedaw-server.vercel.app/allUser/${id}`, { method: "DELETE" })
  ...
  return (
    <div>
      <div className="px-4 pb-5 sm:px-0">
        <h3 className="text-lg font-medium leading-6 text-gray-900">All Buyers</h3>
